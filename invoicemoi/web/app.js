...
(() => {
  ...
  const I18N = {
    fr: {
      ...
      page_title: 'InvoiceMoi — Facturation BTP',
    },
    en: {
      lang_label: 'Language',
      btn_new: 'New',
      ...
      page_title: 'InvoiceMoi — Construction Invoice Builder',
    },
  };
  ...
  async function saveFileWithPicker(blob, suggestedName, description, mimeType, extensions) {
    if (!window.showSaveFilePicker) {
      console.info('InvoiceMoi: showSaveFilePicker unavailable in this browser — using plain download instead.');
    } else if (window.location.protocol === 'file:') {
      console.info('InvoiceMoi: showSaveFilePicker is disabled on file:// pages — serve this over http(s) to get the Save As dialog. Using plain download instead.');
    } else {
      try {
        ...
        console.warn('InvoiceMoi: showSaveFilePicker failed, falling back to plain download.', err);
      }
    }
  ...
  async function generatePdf() {
    ...
    return {
      ...
      filename: `InvoiceMoi-${docTypeFilenameWord()}-${safeInvoiceNumber(state.invoice.number)}.pdf`,
    };
  }
  ...
  $('btnSaveData').addEventListener('click', async () => {
    ...
    const filename = `InvoiceMoi-${docTypeFilenameWord()}-${safeInvoiceNumber(state.invoice.number)}-${fileTimestamp()}.json`;
    const saved = await saveFileWithPicker(blob, filename, 'InvoiceMoi data (JSON)', 'application/json', ['.json']);
    if (saved) {
      isDirty = false;
